...
var errFailedMsg = (specId) => `Failed to update spec ${specId}`;
var errFailedReqMsg = (specId, resp) => `${errFailedMsg(specId)}. ${resp.status} ${resp.statusText}`;
async function run() {
  try {
    ...
    const hostname = core.getInput("hostname") || "api.impartsecurity.net";
    ...
    if (!name) {
      ...
      const getResJson = await getResp.json();
